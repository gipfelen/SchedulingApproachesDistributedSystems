...
import at.uibk.dps.dsB.task4.PropertyProviderDynamicAverage;
...
public class TimingEvaluator implements ImplementationEvaluator {
	...
	protected final PropertyProvider propertyProvider = new PropertyProviderDynamicAverage(new PropertyProviderDynamic(), 1000);
	...
	private Map<Task, Double> calculateEndTime(Specification implementation, Task currentTask, Map<Task, Double> startTimes,
		...
		if (currentTask.getAttribute("End time") != null){
			var currentEndTime = currentTask.getAttribute("End time");
			currentTask.setAttribute("End time", (double) currentEndTime  > endTime ? currentEndTime : endTime);
		} else{
		...
		for (Task successor : appl.getSuccessors(currentTask)) {
			var allPredSet = true;
			for (var pred : appl.getPredecessors(successor)){
				if (pred.getAttribute("End time") == null){
					allPredSet = false;
					break;
				}
			}
